...
void main() {
  ...
  group('StreakService.isBroken', () {
    test('is false with no recorded activity yet', () {
      expect(StreakService.isBroken(lastActivityDate: null, now: DateTime(2026, 1, 10)), isFalse);
    });

    test('is false the same day or the very next day', () {
      expect(
        StreakService.isBroken(lastActivityDate: DateTime(2026, 1, 10), now: DateTime(2026, 1, 10)),
        isFalse,
      );
      expect(
        StreakService.isBroken(lastActivityDate: DateTime(2026, 1, 9), now: DateTime(2026, 1, 10)),
        isFalse,
      );
    });
    ...
    test('is true once a full day has been skipped', () {
      expect(
        StreakService.isBroken(lastActivityDate: DateTime(2026, 1, 8), now: DateTime(2026, 1, 10)),
        isTrue,
      );
    });
  });
